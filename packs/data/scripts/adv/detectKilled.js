...
world.afterEvents.entityDie.subscribe((e) => {
	...
	const allowedMobs =
		deadEntity.matches({ families: ["monster"] }) ||
		deadEntity.matches({ families: ["cow"] });
	if (!damaged.startsWith("minecraft:")) return;
	if (
		damaged == "minecraft:item" ||
		damaged == "minecraft:ender_dragon" ||
		damaged == "minecraft:wither" ||
		!allowedMobs
	)
		return;
...
world.afterEvents.entityHurt.subscribe((e) => {
	...
	const healthPercentage =
		(healthCurrent / hurtEntity.getComponent("health").effectiveMax) * 100;
	if (damaged == "minecraft:item") return;
	if (damaged != "minecraft:ender_dragon" && damaged != "minecraft:wither")
		return;
