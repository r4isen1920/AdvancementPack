...
function setupDialogueHandler(player) {
	...
	if (dialogueHandler.length <= 0) {
		player.dimension.spawnEntity(
			...
			Vector3.add(player.location, new Vector3(0, 8, 0)),
		);
	}
}
...
function openScreen(player, screen) {
	...
	player.runCommand(
		`dialogue open @e[type=adv:dialogue_handler,c=1] @s _r4ui:${screen}_tab`,
	);
...
system.afterEvents.scriptEventReceive.subscribe((event) => {
	...
	switch (message) {
		case "achievements":
			openScreen(sourceEntity, "achievements");
			break;
		case "adventure":
			openScreen(sourceEntity, "adventure");
			break;
		case "husbandry":
			openScreen(sourceEntity, "husbandry");
			break;
		case "end":
			openScreen(sourceEntity, "end");
			break;
		case "nether":
			openScreen(sourceEntity, "nether");
			break;
		case "story":
			openScreen(sourceEntity, "story");
			break;
		default:
			openScreen(sourceEntity, "empty");
			break;
	}
});
