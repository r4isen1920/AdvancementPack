...
export function showAchievementToast(
	...
	if (slideshow == undefined || !slideshow) {
		player.sendMessage(
			`_r4ui:toast_0.header.${typeSet[0]}.body.${id}.slideshow_0.${icon}`,
		);
	} else {
		player.sendMessage(
			`_r4ui:toast_0.header.${typeSet[0]}.body.${id}.slideshow_1.${icon}`,
		);
	}
	...
	player.runCommand(
		`tellraw @a {"rawtext":[{"translate":"${typeSet[1]}","with":{"rawtext":[{"selector":"@s"},{"translate":"r4ui.toast.body.${id}"}]}}]}`,
	);
